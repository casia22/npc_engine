...
class NPC:
    def __init__(self, name, desc, knowledge, mood="正常",ob=[],memory=[],model="gpt-3.5-turbo"):
        self.name = name
        self.desc = desc
        self.mood = mood
        self.memory = memory
        ...
        if self.model.startswith("chatglm"):
            self.memeory[0]["role"] = "user"
            self.memeory.append({"role": "assistant", "content": "好的，下面我会按照您的要求扮演。"})
...
class NPCEngine:
    ...
    def init(self, json_data):
        ...
        for npc_data in npc_list:
            npc = NPC(name = npc_data["name"], desc=npc_data["desc"], mood = npc_data["mood"],knowledge=knowledge, memory = npc_data["memory"]) # todo:👀NPC观察也就是ob没有做
            self.npc_dict[npc.name] = npc
            print("inited npc:", npc.name, npc.desc, npc.mood, npc.knowledge, npc.memory)
